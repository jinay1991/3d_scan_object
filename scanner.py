...
class Scanner():
    ...
    def isWithinROICircle(self, pt):
        ...
        """
        (cx, cy) = pt
        circle_lhs = pow((cx - self.center_x), 2) + pow((cy - self.center_y), 2)
    ...
    def preprocess(self):
        ...
        self.cropped_image = self.in_image.copy()
    ...
    def detect_edges(self, lo_thresh=80, hi_thresh=160):
        ...
        # Morphological Closing Operation to connect the joints and fill the holes/breakages
    ...
    def detect_contours(self):
        ...
        # discard if no continuos contour found [Request User for realign and restart]
        if len(t_contours) == 0:
        ...
        # generate cropped results
        mask = np.zeros((self.rows, self.cols), dtype=np.uint8)
        cv2.drawContours(mask, self.contour_pts, -1, 255, thickness=-1)

        self.cropped_image[mask == 0] = [0, 0, 0]

    def realign_object(self):
        """
        Re-align Detected Object in the Center
        """
        vis_in = np.zeros((self.rows * 2, self.cols * 2, 3), dtype=np.uint8)
        x_displacement = self.cols - self.center_x
        y_displacement = self.rows - self.center_y
        M = np.float32([[1, 0, x_displacement],[0, 1, y_displacement]])
        vis_in = cv2.warpAffine(self.cropped_image, M, (self.cols * 2, self.rows * 2))
        ...
        for i in range(len(self.contour_pts)):
            ...
            x_disp = x_displacement - cx
            y_disp = y_displacement - cy
            M = np.float32([[1, 0, x_disp],[0, 1, y_disp]])
            vis_in = cv2.warpAffine(vis_in, M, (self.cols * 2, self.rows * 2))
    ...
    def draw_results(self, dump_output=True):
        ...
        # draw center point
        cv2.circle(vis_in, (self.center_x, self.center_y), 2, (255, 0, 0), thickness=-1)
        ...
        if log.level == logging.DEBUG:
            ...
            plt.subplot(223), plt.title("cropped"), plt.imshow(self.cropped_image)
...
class MultiScan():
    ...
    def matchPoints(self, image_1, image_2):
        ...
        kp1_image = cv2.drawKeypoints(image_1, kp1, None, color=(0,255,0), flags=0)
        kp2_image = cv2.drawKeypoints(kp1_image, kp2, None, color=(0,0,255), flags=0)
        ...
        matches_image = np.zeros((image_1.shape[0] * 2, image_1.shape[1] * 2), dtype=np.uint8)
        matches_image = cv2.drawMatches(image_1, kp1, image_2, kp2, self.matches[:20], matches_image, flags=2)
    ...
    def scan_all(self, image_dir):
        ...
        for filepath in filelist:
            ...
            if os.path.basename(filepath) == "0.jpg":
                prev_scnr = self.scan_single(filepath)
            else:
                scnr = self.scan_single(filepath)
                self.matchPoints(scnr.cropped_image, prev_scnr.cropped_image)
                prev_scnr = scnr


        utils.convert2video(os.path.join(os.path.abspath(os.path.curdir), "output", os.path.basename(args.input)), 
                                        video_name=os.path.basename(args.input) + "_OUT.avi",
                                        fps=5.0)
...
if __name__ == "__main__":
    ...
    scanner = MultiScan()
    if ".jpg" in os.path.basename(args.input):
        log.info("processing %s", args.input)
        scanner.scan_single(args.input)
    else:
